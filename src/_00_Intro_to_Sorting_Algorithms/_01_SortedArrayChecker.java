...
public class _01_SortedArrayChecker {
     ...
     * and false otherwise.
     */
    public static boolean doubleArraySorted(double[] arrr) {
     ...
     * integers).
     */
    public static boolean charArraySorted(char[] arrrr) {
        for (int i = 0; i < arrrr.length-1; i++) {
     ...
     */
    public static boolean stringArraySorted(String[] arrrrr) {
        for (int i = 0; i < arrrrr.length-1; i++) {
			if(arrrrr[i].compareTo(arrrrr[i+1])==-1) {
				return true;
			}
    }
        return false;
